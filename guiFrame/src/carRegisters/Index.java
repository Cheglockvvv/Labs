...
public class Index implements Serializable, Closeable {
    ...
    IndexOne2N cBrand;
    IndexOne2N cModel;
    IndexOne2One regNumber;

    public void test(CarRegister register) throws KeyNotUniqueException {
        assert (register != null);
        //check requirements for unique keys
        if (cBrand.contains(register.carBrand)) {
            throw new KeyNotUniqueException(register.carBrand);
        }
    }

    public void put(CarRegister register, long value) throws KeyNotUniqueException {
        test(register);
        cBrand.put(register.carBrand, value);
        cModel.put(register.model, value);
        regNumber.put(register.registerNumber, value);
    }

    public Index() {
        cBrand = new IndexOne2N();
        cModel = new IndexOne2N();
        regNumber = new IndexOne2One();
    }
